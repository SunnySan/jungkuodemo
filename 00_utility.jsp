...
public Connection DBConnection(){
	try{
		...
		DataSource ds = (DataSource)envContext.lookup(gcDataSourceName);
...
public String writeToFile(String content, String sAccountId, String sAccountName, String sLoginIP, String sLoginIPPrivate){
	...
	sFilePath = gcLogFilePath + "VMallAdmin_" + getDateTimeNow(gcDateFormatYMD) + "_" + gcCDRFileSerialNo + ".log";
		...
		try{
			...
			*/
			...
			if (aBcc!=null && aBcc.length>0){	//BCC收件人
				...
				message.setRecipients(Message.RecipientType.BCC, mailAddrsBcc);
			}	//if (aBcc!=null && aBcc.length>0){	//BCC收件人
...
public String getManagerMailList(){
	...
	int			i			= 0;
	iColCount = 1;
	sSQL = "select AccountEmail from VM_AccountList where LoginRole='2' or LoginRole='9'";	//取得email清單
...
public String getLastModifiedPersonMail(String sMessageId){
	...
	int			i			= 0;
	iColCount = 1;
	sSQL = "select LastModifiedPerson from VM_MessageList where MessageId='" + sMessageId + "'";
	ht = getDBData(sSQL, iColCount);
	sResultCode = ht.get("ResultCode").toString();
	if (sResultCode.equals(gcResultCodeSuccess)){	//有資料
		s = (String[][])ht.get("Data");
	...
	sSQL = "select AccountEmail from VM_AccountList where LoginId='" + sLastModifiedPerson + "'";	//取得email清單
	ht = getDBData(sSQL, iColCount);
	sResultCode = ht.get("ResultCode").toString();
	if (sResultCode.equals(gcResultCodeSuccess)){	//有資料
		s = (String[][])ht.get("Data");
